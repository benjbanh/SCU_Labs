...
* Description: This program implements the Link State routing algorithm using Dijkstra.
* It reads the network topology and cost table from files, updates the cost table based on user input,
* and calculates the shortest path for each router.
*/
...
void *receive_info(void *arg)
{
    ...
    while (1) {
        ...
        // Unpack packet
        int router_id = ntohl(packet[0]);
        int neighbor_id = ntohl(packet[1]);
        int new_cost = ntohl(packet[2]);
        
        // Update the cost table
        pthread_mutex_lock(&lock);
        costs[router_id][neighbor_id] = new_cost;
        costs[neighbor_id][router_id] = new_cost;
...
void *run_link_state(void *arg)
{
    ...
    while (1) {
        ...
        for (i = 1; i < N; i++) {
            ...
            for (j = 0; j < N; j++)
            {
                if (taken[j] == 0 && costs[spot][j] != INFINITE)
                {
                    pthread_mutex_lock(&lock);
