...
static void swap(void* arr[], int a, int b);
 ...
 * Complexity:	O(m)
 *
 * Description:	Change getElements to return a sorted array by implementing the quicksort algorithm
 */
...
/**
 * sorts the given set from within the bounds low and high
 * O(nlogn)
 */
static void quicksort(SET *sp, void **elts, int low, int high){
	assert(sp != NULL);
	if(low < high){
		int index = partition(sp, elts,low,high);
		quicksort(sp, elts,low, index-1);
		quicksort(sp, elts, index+1, high);
	}
	return;
}

/**
 * Places the pivot for the quicksort algorithm
 * O(n)
static int partition(SET *sp, void **elts, int low, int high){
...
//Swaps 2 elements in the array given their indexes
//O(1)
static void swap(void* arr[], int a, int b){
	void *temp = arr[a]; 
	arr[a] = arr[b];
